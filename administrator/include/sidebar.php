        ...
        <ul class="widget widget-menu unstyled">
            <li>
                ...
                <ul id="togglePages2" class="collapse unstyled">
                    <li>
                        <a href="todays-mr.php">
                            <i class="icon-tasks"></i>
                            Today's Material Requisition
                            <?php
                            $f1 = "00:00:00";
                            $from = date('Y-m-d') . " " . $f1;
                            $t1 = "23:59:59";
                            $to = date('Y-m-d') . " " . $t1;
                            $result = mysqli_query($con, "SELECT * FROM mr where mrDate Between '$from' and '$to'");
                            $num_rows1 = mysqli_num_rows($result); {
                                ?>
                                <b class="label orange pull-right"><?php echo htmlentities($num_rows1); ?></b>
                            <?php } ?>
                    ...
                    <li>
                        <a href="pending-mr.php">
                            <i class="icon-tasks"></i>
                            Pending Material Requisition
                            <?php
                            $status = 'Completed';
                            $ret = mysqli_query($con, "SELECT * FROM mr where status!='$status' || status is null ");
                            $num = mysqli_num_rows($ret); {
                                ?><b class="label orange pull-right"><?php echo htmlentities($num); ?></b>
                            <?php } ?>
                        </a>
                    ...
                    <li>
                        <a href="completed-mr.php">
                            <i class="icon-inbox"></i>
                            Completed MR
                            <?php
                            $status = 'Completed';
                            $rt = mysqli_query($con, "SELECT * FROM mr where status='$status'");
                            $num1 = mysqli_num_rows($rt); {
                                ?><b class="label green pull-right"><?php echo htmlentities($num1); ?></b>
                            <?php } ?>
