...
$row = $order_items_qry->fetch_assoc();
    ...
    $totalPrice = number_format(($row['quantity'] * $row['unit_price']),2);
    $sub_total = 0;
    $sub_total += ($row['quantity'] * $row['unit_price']);
    $discPercent = $supplier['discount_percentage'];
    $discAmt = number_format($supplier['discount_amount'],2);
    $taxPercent = number_format($supplier['tax_percentage']);
    $taxAmt = number_format($supplier['tax_amount'],2);
    $total = number_format(($sub_total - $discAmt),2);
