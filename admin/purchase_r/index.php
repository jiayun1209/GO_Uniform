			...
			<table class="table table-hover table-striped">
				...
				<thead>
					<tr class="bg-navy disabled">
						...
						<th class="px-1 py-1 text-center">Item Details</th>
						<th class="px-1 py-1 text-center">Quantity Request</th>
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT p.*, s.name as sname FROM `purchase_requisition` p inner join `staff` s on p.id  = s.id order by unix_timestamp(p.date_updated) ");
...
<script>
	...
	function delete_pr($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=delete_pr",
			method:"POST",
			data:{id: $id},
			dataType:"json",
			error:err=>{
	...
	function renew_pr($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=renew_pr",
			method:"POST",
			data:{id: $id},
			dataType:"json",
			error:err=>{
